...
- A successful closure permits a new ticket. Authorized staff may reopen a closed channel if its owner has no other active ticket.
...
After deployment, run /setup settings in Discord to use the new dashboard. Existing saved settings remain in place. Use Post / refresh panel if automatic panel refresh reports a permission problem. To test, open a Support ticket, close and confirm, then open a new one. Also test staff reopening of a closed Management ticket when its owner has no other active ticket.
